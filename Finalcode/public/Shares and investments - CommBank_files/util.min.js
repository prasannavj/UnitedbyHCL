...
}if(typeof obj!=="object"||"nodeType" in obj){return
...
}}}}function pullContent(path,mboxName){path=CQ.shared.HTTP.addParameter(path,"wcmmode","disabled");
...
if(!mboxParameters){caller=caller.arguments.callee.caller;
continue
}var i,entry;
for(i=0;
...
},pull:function(path,mboxName,version){pullContent(path,mboxName)
